...
#define MFRC522_RST  P1_0
#define MFRC522_MISO P1_1
#define MFRC522_MOSI P1_2
#define MFRC522_SCK  P1_3
#define MFRC522_CS	 P1_4
...
#define PICC_INCREMENT       0xC1               // reunsigned charge
...
//SPI read
   for(i = 8; i > 0; i--)
   {
      MFRC522_SCK = 1;
      ucResult <<= 1;
      ucResult|= (unsigned char)MFRC522_MISO;
      MFRC522_SCK = 0;
   }
...
static void MFRC522_Clear_Bit( unsigned char addr, unsigned char mask )
{     unsigned char  tmp =0x0; 
      tmp=MFRC522_Rd( addr ) ;
     MFRC522_Wr( addr,  tmp&~mask );     // tmp&(~mask)
}
static void MFRC522_Set_Bit( unsigned char addr, unsigned char mask )
{    unsigned char  tmp =0x0; 
      tmp=MFRC522_Rd( addr ) ;  
...
void MFRC522_CRC(unsigned char *dataIn, unsigned char length, unsigned char *dataOut )
{
unsigned char i, n;
 ...
 //Escreve dados no FIFO        
    ...
    i = 0xFF;
    //Espera a finaliza褯 do Calculo do CRC
    do 
    {
...
unsigned char MFRC522_Auth(unsigned char authMode,
													 ...
													 unsigned char *serNum)
{
  ...
  if ( ( status != MI_OK ) || !( MFRC522_Rd( STATUS2REG ) & 0x08 ) )
  {
    status = MI_ERR;
  }
...
unsigned char MFRC522_ReadCardSerial(unsigned char *id)
{
	unsigned char status;
